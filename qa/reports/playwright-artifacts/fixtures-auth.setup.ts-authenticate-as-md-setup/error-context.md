...
- Name: fixtures\auth.setup.ts >> authenticate as md
- Location: fixtures\auth.setup.ts:13:8

# Error details

```
Error: Login as md (md@mrtracker.com) did not leave /login -- check the seed accounts exist (cd web && npm run db:seed) or set E2E_MD_EMAIL / _PASSWORD.
...
Call log:
  - Login as md (md@mrtracker.com) did not leave /login -- check the seed accounts exist (cd web && npm run db:seed) or set E2E_MD_EMAIL / _PASSWORD. with timeout 20000ms
    41 × locator resolved to <html lang="en" class="__variable_f367f3 __variable_ed3508">…</html>
...
- text: Invalid credentials. Please check your password. Email
- textbox: md@mrtracker.com
- text: Password
- textbox: md12345
- button "Sign in"
