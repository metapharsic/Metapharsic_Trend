...
`npm test` runs **76 unit tests across 7 suites**, all passing. Coverage by phase QA gate:
...
**Not yet covered** (verified manually this session, not automated):
- API route handlers end-to-end — needs a test database and request harness.
...
- The 80% coverage target in §3 is not measured; no coverage gate is enforced in CI (no CI pipeline exists).
