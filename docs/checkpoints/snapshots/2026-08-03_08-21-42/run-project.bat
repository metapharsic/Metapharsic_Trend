@echo off
title Pharma OS Suite Runner with Logging
color 0B
...
echo.
echo [LOGS LOCATION]:
echo  - Frontend Mobile Logs  : logs\frontend\
echo  - Backend Database Logs : logs\backend\
echo.
...
start "Pharma OS - Web Dashboard" powershell -NoProfile -ExecutionPolicy Bypass -Command "cd web; npm run dev | Tee-Object -FilePath '..\logs\backend\web.log'"
timeout /t 3 /nobreak >nul

:: 3. Launch Mobile Expo Packager with Tee-Object Logging
echo [3/3] Starting Mobile Expo Packager with logging...
start "Pharma OS - Mobile Client" powershell -NoProfile -ExecutionPolicy Bypass -Command "cd mobile; npm run start | Tee-Object -FilePath '..\logs\frontend\mobile.log'"
...
echo  - Console outputs are mirrored to 'logs\backend\web.log' and 'logs\frontend\mobile.log'.
echo =======================================================================
echo.
pause
