...
const REPORTS = {
  "product-wise-sales": {
    ...
    description: "Units and revenue per product SKU for the current month.",
    run: async () => {
      const items = await db.orderItem.findMany({
        where: { order: { createdAt: { gte: startOfUtcMonth() } } },
        include: { product: { select: { id: true, name: true, sku: true } } },
      });
      const byProduct = new Map<string, { name: string; sku: string; units: number; revenue: number }>();
...
export const GET = withAuth(handleReports, [Role.ASM, Role.ADMIN]);
