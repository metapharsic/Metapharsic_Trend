...
/**
 * Read-only list. Resolving an APPROVED claim (COMPLETED/REJECTED, with an
 * optional credit note) is PUT /api/distributor/claims/[id] — this route only
 * surfaces what's on record. Raising a claim (MR-side) and the PENDING_MR /
 * PENDING_ASM audit steps are still separate, unbuilt scope.
 */
