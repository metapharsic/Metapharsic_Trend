...
async function createOrder(req: AuthedRequest) {
  try {
    ...
    const { order, invoice } = await db.$transaction(async (tx) => {
      ...
      // last manual stock-take. Guarded by stockQty >= quantity in the WHERE
      // clause so a concurrent booking can never push stock negative.
      ...
      // Auto-post the sale to the ledger in the same transaction as the invoice
      // itself — a rolled-back order/invoice must never leave a ledger entry behind.
      const gstAmount = Number(gstTotals.totalGst ?? 0);
      const salesAmount = Number(gstTotals.grandTotal ?? newOrderValue) - gstAmount;
      await postAutoLedger(tx, {
        sourceType: "INVOICE",
        sourceId: createdInvoice.id,
        date: createdInvoice.createdAt,
        narration: `Invoice ${createdInvoice.invoiceNo} — ${chemist.billingName || chemist.name}`,
        lines: [
          { accountCode: SYSTEM_ACCOUNT_CODES.accountsReceivable, debit: Number(gstTotals.grandTotal ?? newOrderValue) },
