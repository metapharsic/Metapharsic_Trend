...
### 5. Accounts / Finance (Planned — see docs/03_features/module_accounts.md)
- `POST/GET /api/finance/accounts`
  - Input (POST): `{ code, name, type, parentId? }`
  - Output: `{ success: true, account: ChartOfAccount }`
- `POST/GET /api/finance/journal-entries`
  - Input (POST): `{ date, narration, entries: [{accountId, debit, credit}] }` (sum debit == sum credit)
  - Output: `{ success: true, transactionId: UUID }`
- `GET /api/finance/reports/trial-balance`
- `GET /api/finance/reports/pnl?from&to`
- `GET /api/finance/reports/balance-sheet?asOf`
  - Output (all reports): server-computed aggregates from `LedgerEntry`, grouped by account — never client-summed.
