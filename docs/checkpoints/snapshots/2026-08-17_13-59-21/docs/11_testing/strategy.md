...
`npm test` runs **171 tests across 13 suites**, all passing (~30s). Coverage by phase QA gate:
...
| `api-workflows.test.ts` | TP approval gating DCR, geofence enforcement, CQS scoring, discount application, duplicate receipts, leave freezing the calendar, KPI updates, MR dashboard tiles and drill-down authorization | Phases 2–4 |
| `mr-dashboard.test.ts` | GPS status decision table (incl. future-dated-fix regression), travel distance excluding spoofed fixes, all 10 notification rules and their thresholds | MR dashboard |
| `hierarchy.test.ts` | Growth/achievement percent edge cases (zero base, zero target), dense ranking with ties, role filtering | RM dashboard |
| `order-workflow.test.ts` | Every order status transition (forward, skip-stage, backward, no-op, final-state), credit utilization null/zero-limit guards | Distributor Portal |
