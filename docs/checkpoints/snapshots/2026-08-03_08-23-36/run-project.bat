@echo off
title Pharma OS Suite Runner with Centralized Logging
color 0B
...
echo.
echo [LOGS LOCATION]:
echo  - Frontend Mobile Logs  : C:\Trend_MR\logs\frontend\
echo  - Backend Database Logs : C:\Trend_MR\logs\backend\
echo.
...
:: Create absolute logs folder structure if they do not exist
if not exist C:\Trend_MR\logs\frontend mkdir C:\Trend_MR\logs\frontend
if not exist C:\Trend_MR\logs\backend mkdir C:\Trend_MR\logs\backend
...
start "Pharma OS - Web Dashboard" powershell -NoProfile -ExecutionPolicy Bypass -Command "cd web; npm run dev | Tee-Object -FilePath 'C:\Trend_MR\logs\backend\web.log'"
timeout /t 3 /nobreak >nul

:: 3. Launch Mobile Expo Packager with Tee-Object Logging
echo [3/3] Starting Mobile Expo Packager with logging...
start "Pharma OS - Mobile Client" powershell -NoProfile -ExecutionPolicy Bypass -Command "cd mobile; npm run start | Tee-Object -FilePath 'C:\Trend_MR\logs\frontend\mobile.log'"
...
echo  - Terminal logs are mirrored to 'C:\Trend_MR\logs\backend\web.log' and 'C:\Trend_MR\logs\frontend\mobile.log'.
echo =======================================================================
echo.
pause
