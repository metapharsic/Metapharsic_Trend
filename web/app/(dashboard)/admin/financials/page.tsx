...
export default function AdminFinancialsPage() {
  ...
  useEffect(() => {
    apiClient
      ...
      .then((res) => setLedgers(res.data.data.ledgers))
  ...
  const totals = ledgers.reduce(
    (acc, l) => {
      acc.salesRevenue += Number(l.salesRevenue ?? 0);
      acc.creditGiven += Number(l.creditGiven ?? 0);
      acc.amountCollected += Number(l.amountCollected ?? 0);
      acc.outstandingCredit += Number(l.outstandingCredit ?? 0);
      acc.netRealizedProfit += Number(l.netRealizedProfit ?? 0);
      return acc;
    },
    { salesRevenue: 0, creditGiven: 0, amountCollected: 0, outstandingCredit: 0, netRealizedProfit: 0 }
  );
