...
describe("pricing ladder round-trip", () => {
  ...
  it("round-trips purchase rate 100 through MRP and back (PTS deferred)", () => {
    ...
    expect(forward.ptr).toBeCloseTo(140, 1);
    expect(forward.mrp).toBeCloseTo(175, 1);
  ...
  it("maintains purchaseRate < ptr < mrp for realistic inputs", () => {
    const cases = [
      { purchaseRate: 12.5, companyMarginPct: 35, chemistMarginPct: 20 },
      { purchaseRate: 100, companyMarginPct: 40, chemistMarginPct: 20 },
      { purchaseRate: 3.4, companyMarginPct: 60, chemistMarginPct: 16 },
      { purchaseRate: 875, companyMarginPct: 25, chemistMarginPct: 24 },
    ];

    for (const input of cases) {
      const r = ProductPricingAgentsService.calculatePricing({ ...input, autoCalculate: true });
      if (!(r.purchaseRate < r.ptr && r.ptr < r.mrp)) {
        throw new Error(
          `Hierarchy violated for ${JSON.stringify(input)}: ` +
            `purchaseRate=${r.purchaseRate} ptr=${r.ptr} mrp=${r.mrp}`
        );
      }
  ...
  it("produces no Infinity or NaN when purchase rate is zero or missing", () => {
    const inputs = [
      ...
      { ptr: 60, mrp: 80, autoCalculate: false },
    ];
