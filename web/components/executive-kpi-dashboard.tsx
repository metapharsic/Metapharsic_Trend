...
/**
 * Org-wide KPI overview — same underlying data for Admin and MD, since both
 * need the full national picture, not a per-territory slice. Title/subtitle
 * differ per role, the numbers don't.
 */
export function ExecutiveKpiDashboard({ title, subtitle }: { title: string; subtitle: string }) {
  ...
  if (!kpis) {
    return (
      <div className="bg-white rounded-2xl p-12 text-center border border-gray-100">
        <p className="text-gray-400 text-sm">Unable to load KPIs.</p>
      </div>
    );
  }
